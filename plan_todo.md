...
- [x] **懸浮搜尋列 (Floating Search Bar)**：
    - [x] 將搜尋列改為懸浮在日誌檢視區右上角，不再推擠內容佈局。
    - [x] 加入圓角、陰影與微小的淡入淡出動畫。
- [x] **快速捷徑優化**：
    - [x] 在搜尋列顯示時，按下 `Esc` 僅關閉搜尋列而不清除目前的選取項。
...
- [x] **搜尋結果可視化**：
    - [x] 在垂直捲軸背景或旁邊，繪製搜尋匹配項的色塊分佈。

### 4. 狀態列與互動性 (Interactive Status Bar)
- [x] **點擊導向**：
    - [x] 點擊「總行數」區域 -> 直接開啟 `Go To Line`。
    - [x] 點擊「過濾器狀態」 -> 快速切換 `Show Filtered Only`。
- [x] **繁忙指示 (Loading Spinner)**：
    - [x] 在 Rust 背景處理大檔案或複雜過濾時，顯示小型的動畫指示器。
...
### 7. Activity Bar 精緻化
- [x] **通知徽章 (Badges)**：
    - [x] 在 Filter 圖標上顯示目前啟用的過濾器數量。
    - [x] 在 Notes 圖標上顯示目前檔案的筆記總數。
